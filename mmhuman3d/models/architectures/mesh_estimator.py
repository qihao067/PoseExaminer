...
class BodyModelEstimator(BaseArchitecture, metaclass=ABCMeta):
    ...
    def run_registration(
        ...
        # LQH: adv_training 
        # FOR Adv Training of SPIN: - we do not need 2d keypoints, and 3D joints come from smpl model
        if targets['keypoints3d'].shape[1]!=49:
            import pdb; pdb.set_trace()
        
        has_keypoints3d = targets['has_keypoints3d'].squeeze(-1)
        targets['keypoints2d'] = targets['keypoints2d'][:,:49,:]
        gt_model_joints_cat = torch.cat((gt_model_joints,torch.ones([gt_model_joints.shape[0],gt_model_joints.shape[1],1]).to(gt_model_joints.device)),-1).double()
        targets['keypoints3d'][has_keypoints3d==2] = gt_model_joints_cat[has_keypoints3d==2]
        has_keypoints3d[has_keypoints3d==2]=1
        targets['has_keypoints3d'] = has_keypoints3d.unsqueeze(-1)
        no_cam = True
        
        # if True or targets['keypoints2d'].shape[1]!=49:
        #     targets['keypoints2d'] = targets['keypoints2d'][:,:49,:]
        #     targets['keypoints3d'] = targets['keypoints3d'][:,:49,:]
        #     no_cam = True
        # else:
        #     no_cam = False
    ...
    def compute_keypoints3d_loss(
        ...
        # # LQH: adv testing - AT-V2 版本，这个版本不用3D keypoint
        # loss = self.loss_keypoints3d(
        #     pred_keypoints3d, pred_keypoints3d, reduction_override='none')
    ...
    def compute_keypoints2d_loss(
        ...
        # # LQH: adv testing
        # loss = self.loss_keypoints2d(
        #     pred_keypoints2d, pred_keypoints2d, reduction_override='none')
    ...
    def compute_losses(self, predictions: dict, targets: dict):
        ...
        if 'valid_fit' in targets:
            ...
            gt_model_joints = gt_keypoints3d[:,:,:3].clone() #LQH: adv testing: 这里应该在regist的时候就用smpl配好了
        else:
        ...
        losses = {}
        # LQH : adv training: currently we do not use 2d keypoints
        if self.loss_keypoints3d is not None:
            # LQH: adv testing: AT-V3 之后的版本，使用了smpl参数fit出来的3d keypoint作为gt
            gt_keypoints3d_temp = torch.cat((gt_model_joints,torch.ones([gt_model_joints.shape[0],gt_model_joints.shape[1],1]).to(gt_model_joints.device)),-1).double()
            gt_keypoints3d[has_keypoints3d==2] = gt_keypoints3d_temp[has_keypoints3d==2]
            has_keypoints3d[has_keypoints3d==2] = 1
        ...
        if self.loss_keypoints2d is not None:
            # LQH: adv testing: AT-V3 之后的版本，2d keypoint 直接变成0
            gt_keypoints2d[has_keypoints2d==2] = gt_keypoints2d[has_keypoints2d==2] * 0
            has_keypoints2d[has_keypoints2d==2] = 0
