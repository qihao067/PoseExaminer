...
class HumanImageDataset(BaseDataset, metaclass=ABCMeta):
    ...
    def get_annotation_file(self):
        ...
        # LQH: adv_training 
        if self.data_prefix[:3]=='exp':
            ann_prefix = self.data_prefix
        else:
            ann_prefix = os.path.join(self.data_prefix, 'preprocessed_datasets')
            # ann_prefix = os.path.join(self.data_prefix, 'preprocessed_datasets') # original
    ...
    def prepare_raw_data(self, idx: int):
        ...
        # LQH-adv testing
        if self.data_prefix[:3]=='exp':
            info['image_path'] = os.path.join('exps',image_path)
        else:
        ...
        if self.data_prefix[:3]=='exp': # LQH-adv testing : 为了加入 3d loss，这里强行把 has_keypoint 变成 true
            info['has_keypoints3d'] = 2
            info['has_keypoints2d'] = 2
    ...
    def evaluate(self,
        ...
        for _metric in metrics:
            if _metric == 'mpjpe':
                _nv_tuples = self._report_mpjpe(res, output_ver=output_ver)
            elif _metric == 'pa-mpjpe':
                _nv_tuples = self._report_mpjpe(res, metric='pa-mpjpe', output_ver=output_ver)
            elif _metric == '3dpck':
            ...
            elif _metric == 'mpjpe-2d': # LQH-adv testing
                _nv_tuples = self._report_mpjpe(res, metric='mpjpe-2d', output_ver=output_ver)
            else:
    ...
    def _parse_result(self, res, mode='keypoint', body_part=None):
        ...
        if mode == 'vertice':
            ...
            if gt_beta.shape[1]!=10: # LQH: TODO: need to fix!!
                gt_beta = torch.zeros((gt_beta.shape[0],10))
            ...
            if self.dataset_name in ['pw3d','adv','aist','doh3']: # LQH-adv testing
                betas = []
                body_pose = []
                global_orient = []
                gender = []
                ...
                if betas.shape[1]!=10: # LQH: adv testing : aist: 没有shape参数，而是用的scale, 而且相机外参数有旋转
                    gt_output = self.body_model(
                        body_pose=body_pose,
                    ...
                    #     global_orient=global_orient,
                    #     gender=gender)
                    # 但是其实用不用transl意义不大，因为都是用regressor得到的 3d kpt
                    gt_output['joints'][:,:,1:] = -gt_output['joints'][:,:,1:] # AIST 的相机0绕x轴旋转了180度，这是简化般的旋转变化 # TODO: 这里需要修改一下
                else:
                    gt_output = self.body_model(
                        betas=betas,
                        body_pose=body_pose,
    ...
    def _report_mpjpe(self, res_file, metric='mpjpe', body_part='', output_ver='None'):
        ...
        elif metric == 'mpjpe-2d': # LQH-adv testing
            alignment = 'mpjpe-2d'
        else:
            raise ValueError(f'Invalid metric: {metric}')

        error = keypoint_mpjpe(pred_keypoints3d, gt_keypoints3d,
                               gt_keypoints3d_mask, alignment, output_ver)
