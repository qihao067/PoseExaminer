...
def build_dataset(cfg: Union[dict, list, tuple], proc_id: int = -1,
    ...
    # LQH: adv testing
    try:
        if cfg.data_prefix[:3]=='exp' and cfg.ann_file[:4]=='data': # only for adv_test
            cfg.ann_file = 'Proc_'+str(proc_id)+'/'+cfg.ann_file
    except:
        temp_len = len(cfg['configs'])
        for i in range(temp_len):
            if cfg['configs'][i].data_prefix[:3]=='exp' and cfg['configs'][i].ann_file[:4]=='data':
                cfg['configs'][i].ann_file = 'Proc_'+str(proc_id)+'/'+cfg['configs'][i].ann_file
