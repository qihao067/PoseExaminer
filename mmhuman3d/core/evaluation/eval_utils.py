...
def keypoint_mpjpe(pred, gt, mask, alignment='none', output_ver='None'):
    ...
    elif alignment == 'mpjpe-2d':
        # To speed up training process, we use pseudo 2d mpjpe when studing the articulated pose.
        # We can use this because R is identity matrix.
        if output_ver!='None':
            error = np.linalg.norm(pred[:,:,:2] - gt[:,:,:2], ord=2, axis=-1)
        else:
            error = np.linalg.norm(pred[:,:,:2] - gt[:,:,:2], ord=2, axis=-1)[mask].mean()
        return error
    else:
    ...
    if output_ver=='None':
        ...
        error = np.linalg.norm(pred - gt, ord=2, axis=-1) # adv testing
