...
max_epoch = 1 #3 #5 #50
batch_size = 32
num_workers = 16 # 16 # 1:用于adv testing，因为每次数据少，workers不用那么多；
lr_coeff = 0.05 # 0.02 # 0.1
baseline_epoch = 12

# optimizer
optimizer = dict(
    backbone=dict(type='Adam', lr=5.0e-5*lr_coeff),
    head=dict(type='Adam', lr=5.0e-5*lr_coeff),
)
...
checkpoint_config = dict(interval=1) # original: 6
...
model = dict(
    ...
    loss_keypoints2d=dict(type='MSELoss', loss_weight=0.0), # Original: 150
    loss_smpl_pose=dict(type='MSELoss', loss_weight=60),
    loss_smpl_betas=dict(type='MSELoss', loss_weight=60 * 0.001),
    loss_camera=dict(type='CameraPriorLoss', loss_weight=0.0), # original: 1
    init_cfg=dict(
        type='Pretrained',
...
data = dict(
    ...
    train=dict(
        ...
        configs=[
            ...
            dict(
                ...
                pipeline=train_pipeline,
                convention='smpl_24',
                ann_file='spin_adv_train.npz'),
        ],
        partition=[0.46, 0.213, 0.036, 0.011, 0.18, 0.1], #[0.45, 0.203, 0.026, 0.01, 0.161, 0.15], #[0.5, 0.233, 0.046, 0.021, 0.2]
        num_data=100000,
    ),
    # test=dict(
    #     type=dataset_type,
    ...
    adv_test=dict(
        ...
        dataset_name='adv',
        data_prefix='exps',
        pipeline=adv_test_pipeline,
        ann_file='data_generated/generated_mmpose.npz'),
)
